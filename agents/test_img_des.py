...
def main():
    ...
    damage_defective_status_agent = autogen.AssistantAgent(
        ...
        system_message="""if the image is of package then you will use the description of the image from image_explainer agent and give the final one decision out of below along with its description as well as the image url proper markdown format.
            1) Refund: if package seems seriously damaged then you will provide the refund to the customer.
            2) Replace: if package is wet then you will replace the package
    ...
    image_explainer = MultimodalConversableAgent(
        ...
        llm_config=llm_config,
        system_message="for any request related to condition, status or description of image then I will give the detailed description of the image",
    )
